...
import generate.ByteArrayClassLoader;
import generate.GenerateFacadeAndRequestResponse;
...
public class Compile {
    ...
    public static Class<?> doCompile(String className,String content,CompileOptions compileOptions,Boolean compileWithAnotherClassLoader,File classFile){
        ...
        try {
            ...
            if (Reflect.CACHED_LOOKUP_CONSTRUCTOR != null && !compileWithAnotherClassLoader) {
                ClassLoader finalCl = cl;
                result = fileManager.loadAndReturnMainClass(className,
                        (name, bytes) -> Reflect.on(finalCl).call("defineClass", name, bytes, 0, bytes.length).get());
            }

            if (compileWithAnotherClassLoader){
                result = fileManager.loadAndReturnMainClass(className);
            }

            if (classFile != null){
                fileManager.generateClassFile(className,classFile);
            }
    ...
    public static Class<?> compile(String className, String content, CompileOptions compileOptions) {
        ...
        try {
            ...
            return doCompile(className,content,compileOptions,false,null);
        }
    }
    ...
    static final class ClassFileManager extends ForwardingJavaFileManager<StandardJavaFileManager> {
        ...
        void generateClassFile(String mainClassName,File file) throws Exception {
        ...
        Class<?> loadAndReturnMainClass(String mainClassName) throws Exception {
            Class<?> result = null;

            for (Entry<String, byte[]> entry : classes().entrySet()) {
                ByteArrayClassLoader byteArrayClassLoader = new ByteArrayClassLoader(GenerateFacadeAndRequestResponse.class.getClassLoader(), entry.getKey(), entry.getValue());
                Class<?> c = byteArrayClassLoader.loadClass(entry.getKey());
                if (mainClassName.equals(entry.getKey()))
                    result = c;
            }
