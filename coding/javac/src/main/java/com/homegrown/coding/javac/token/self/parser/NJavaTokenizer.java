...
public class NJavaTokenizer {
    ...
    /** The token factory. */
    private final NTokens tokens;

    /** The token kind, set by nextToken().
     */
    protected NTokens.NTokenKind tk;
    ...
    /** Read token.
     */
    public NTokens.NToken readToken() {
        ...
        int pos = 0;
        int endPos = 0;
        List<NTokens.NComment> comments = null;
        ...
        try {
                ...
                switch (reader.ch) {
                    ...
                    case '.':
                        ...
                        if ('0' <= reader.ch && reader.ch <= '9') {
                            ...
                            if (reader.ch == '.') {
                                reader.scanChar();
                                reader.putChar('.');
                                tk = NTokens.NTokenKind.ELLIPSIS;
                            } else {
                                lexError(savePos, "illegal.dot");
                            }
                        } else {
                            tk = NTokens.NTokenKind.DOT;
                        }
                        break loop;
                    case ',':
                        reader.scanChar(); tk = NTokens.NTokenKind.COMMA; break loop;
                    case ';':
                        reader.scanChar(); tk = NTokens.NTokenKind.SEMI; break loop;
                    case '(':
                        reader.scanChar(); tk = NTokens.NTokenKind.LPAREN; break loop;
                    case ')':
                        reader.scanChar(); tk = NTokens.NTokenKind.RPAREN; break loop;
                    case '[':
                        reader.scanChar(); tk = NTokens.NTokenKind.LBRACKET; break loop;
                    case ']':
                        reader.scanChar(); tk = NTokens.NTokenKind.RBRACKET; break loop;
                    case '{':
                        reader.scanChar(); tk = NTokens.NTokenKind.LBRACE; break loop;
                    case '}':
                        reader.scanChar(); tk = NTokens.NTokenKind.RBRACE; break loop;
                    case '/':
                        reader.scanChar();
                        if (reader.ch == '/') {
                            do {
                            ...
                            if (reader.bp < reader.buflen) {
                                comments = addComment(comments, processComment(pos, reader.bp, NTokens.NComment.NCommentStyle.LINE));
                            }
                            break;
                        } else if (reader.ch == '*') {
                            boolean isEmpty = false;
                            reader.scanChar();
                            NTokens.NComment.NCommentStyle style;
                            if (reader.ch == '*') {
                                style = NTokens.NComment.NCommentStyle.JAVADOC;
                                reader.scanCommentChar();
                                if (reader.ch == '/') {
                                    isEmpty = true;
                                }
                            } else {
                                style = NTokens.NComment.NCommentStyle.BLOCK;
                            }
                            ...
                            tk = NTokens.NTokenKind.SLASHEQ;
                            reader.scanChar();
                        } else {
                            tk = NTokens.NTokenKind.SLASH;
                        }
                        break loop;
                    case '\'':
                        reader.scanChar();
                        if (reader.ch == '\'') {
                            ...
                            if (reader.ch == '\'') {
                                reader.scanChar();
                                tk = NTokens.NTokenKind.CHARLITERAL;
                            } else {
                    ...
                    case '\"':
                        ...
                        while (reader.ch != '\"' && reader.ch != CR && reader.ch != LF && reader.bp < reader.buflen)
                            scanLitChar(pos);
                        if (reader.ch == '\"') {
                            tk = NTokens.NTokenKind.STRINGLITERAL;
                            reader.scanChar();
                        } else {
            ...
            endPos = reader.bp;
            switch (tk.tag) {
                case DEFAULT: return new NTokens.NToken(tk, pos, endPos, comments);
                case NAMED: return new NTokens.NNamedToken(tk, pos, endPos, name, comments);
                case STRING: return new NTokens.NStringToken(tk, pos, endPos, reader.chars(), comments);
                case NUMERIC: return new NTokens.NNumericToken(tk, pos, endPos, reader.chars(), radix, comments);
    ...
    //where
    List<NTokens.NComment> addComment(List<NTokens.NComment> comments, NTokens.NComment comment) {
    ...
    private void scanOperator() {
        while (true) {
            ...
            NTokens.NTokenKind tk1 = tokens.lookupKind(newname);
            if (tk1 == NTokens.NTokenKind.IDENTIFIER) {
                reader.sp--;
                break;
            }
            tk = tk1;
     ...
     */
    protected NTokens.NComment processComment(int pos, int endPos, NTokens.NComment.NCommentStyle style) {
        if (scannerDebug)
        ...
        return new NJavaTokenizer.NBasicComment<NUnicodeReader>(new NUnicodeReader(fac, buf, buf.length), style);
    }
    ...
    private void scanHexExponentAndSuffix(int pos) {
        ...
        if (reader.ch == 'f' || reader.ch == 'F') {
            reader.putChar(true);
            tk = NTokens.NTokenKind.FLOATLITERAL;
            radix = 16;
        } else {
            if (reader.ch == 'd' || reader.ch == 'D') {
                reader.putChar(true);
            }
            tk = NTokens.NTokenKind.DOUBLELITERAL;
            radix = 16;
        }
    }
    ...
    private void scanNumber(int pos, int radix) {
        ...
        if (radix == 16 && reader.ch == '.') {
            ...
            if (reader.ch == 'l' || reader.ch == 'L') {
                reader.scanChar();
                tk = NTokens.NTokenKind.LONGLITERAL;
            } else {
                tk = NTokens.NTokenKind.INTLITERAL;
            }
        }
    }
     ...
     */
    protected static class NBasicComment<U extends NUnicodeReader> implements NTokens.NComment {
        ...
        protected NBasicComment(U comment_reader, NCommentStyle cs) {
        ...
        public NCommentStyle getStyle() {
            return cs;
        }

        public boolean isDeprecated() {
            if (!scanned && cs == NCommentStyle.JAVADOC) {
                scanDocComment();
            }
    ...
    public static void main(String[] args) {
        String code = "package com.homegrown.coding.javac.token;\n" +
                "\n" +
                "/**\n" +
                " * @author jam\n" +
                " * @description\n" +
                ...
                "    }\n" +
                "}";
        NScannerFactory scannerFactory = new NScannerFactory(new Context());
        String value = "i'm from China";
        NUnicodeReader unicodeReader = new NUnicodeReader(scannerFactory, CharBuffer.wrap(code));
        NJavaTokenizer javaTokenizer = new NJavaTokenizer(scannerFactory, unicodeReader);
        NJavaTokenizer.scannerDebug=true;
        while (javaTokenizer.reader.bp<javaTokenizer.reader.buflen){
            NTokens.NToken token = javaTokenizer.readToken();
//            System.out.println(token.kind+"===="+token.pos+"_"+token.endPos);
        }
    }
}
